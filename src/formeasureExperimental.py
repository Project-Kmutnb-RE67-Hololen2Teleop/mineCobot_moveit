...
class DEFAULT_VAR(Enum):
    ...
    OFSET_MANIP_Y = 0
    OFSET_MANIP_Z = 12.7    #ระยะสูงจาก center ของ mobile ไป centerฐานของ manipulator
                ...
                if SUB_TRIGGER == True :
                    ...
                    for i in range(len(data)):
                        ...
                        if i == 0 :
                            ...
                            theshold = str(input("that enough (Y/N):"))  # นำมาใส่ก่อนการเริ่มลูป
                            while theshold.lower() == "n" or theshold.lower() != "y" :  # เช็คว่าเป็น 'y' หรือ 'Y' โดยใช้ .lower() เปลี่ยนเป็นตัวพิมพ์เล็ก
                                xxx = float(input("Insert PICK Location X :"))
                                yyy = float(input("Insert PICK Location Y :"))
                                Yaww = float(input("Insert PICKRotation Yw :"))
                                BASE = [xxx , yyy]
                                YAW = Yaww
                                transforms.setBaseManip([BASE[0] + DEFAULT_VAR.OFSET_MANIP_X.value, (-BASE[1]), DEFAULT_VAR.OFSET_MANIP_Z.value], [0, -0, YAW])
                                matrix = transforms.outputPosition()
                                ...
                                x, y, z = matrix[:3, 3]
                                move_to_pose(group, round(x/100, DEFAULT_VAR.DIGIT_CTRL.value), round((y/100)*-1, DEFAULT_VAR.DIGIT_CTRL.value), round(((z+DEFAULT_VAR.OFSET_UP.value)/100), DEFAULT_VAR.DIGIT_CTRL.value), Rr , Rp , Ry) 
                                theshold = str(input("that enough (Y/N):"))
                            time.sleep(5)
                        ...
                        if i == len(data) - 1 :
                            ...
                            while theshold.lower() == "n" or theshold.lower() != "y":  # เช็คว่าเป็น 'y' หรือ 'Y' โดยใช้ .lower() เปลี่ยนเป็นตัวพิมพ์เล็ก
                                ...
                                xxx = float(input("Insert PLACE Location X :")) ; yyy = float(input("Insert PLACE Location Y :"))
                                Yaww = float(input("Insert PLACERotation Yw :"))
                                ZZZ = float(input("Insert PLACERotation ZZZ :"))
                                BASE = [xxx , yyy]
                                YAW = Yaww
                                transforms.setBaseManip([BASE[0] + DEFAULT_VAR.OFSET_MANIP_X.value,(-BASE[1]),DEFAULT_VAR.OFSET_MANIP_Z.value] , [0,-0,YAW])
                                matrix = transforms.outputPosition()
                                ...
                                move_to_pose(group, Lx/100 , -(Ly/100), ZZZ ,LRr , LRp , LRy )   
                                theshold = input("that enough (Y/N):")
                            #time.sleep(5)
